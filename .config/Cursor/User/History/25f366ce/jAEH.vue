      ...
      <div class="card w-full max-w-md bg-base-100 shadow-xl">
        <div class="card-body">
          ...
          <div v-if="!todayIntention" class="space-y-4">
            <input v-model="newIntention" type="text" placeholder="Today, I want to..."
              class="input input-bordered w-full" />
            <button @click="saveIntention" :disabled="!newIntention" class="btn btn-primary w-full">
              Create
            </button>
          </div>
          ...
          <div v-else class="space-y-4">
            <div class="text-center">
              ...
              <button v-if="!todayIntention.done" @click="markAsDone" class="btn btn-primary mt-4">
                Mark as done
              </button>
          ...
          <div class="space-y-4">
            ...
            <div v-for="(intention, date) in lastSevenDays" :key="date" class="flex items-center gap-2">
              ...
              <div class="w-4 h-4 rounded-full" :class="[
                intention.done
                  ? (isDarkMode
                    ? 'bg-green-500 border border-white/70'
                    : 'bg-success')
                  : (isDarkMode
                    ? 'bg-base-100 border border-white/20'
                    : 'bg-base-300')
              ]"></div>
            </div>
          </div>
